    ...
    <div class="min-h-screen relative" style="background-image: url('/images/background-mc.png');">
        ...
        <div class="container mx-auto px-20 py-8 mt-5">
            <div class="flex flex-col lg:flex-row gap-6">
                {{-- SIDEBAR --}}
                <div class="w-full grid grid-cols-1 gap-3 w-2/6 xl:w-2/6 lg:w-2/4 md:w-2/2 sm:w-2/2 rounded-lg text-center shadow max-h-30  mb-15">
                    <div class="flex rounded-lg flex-col bg-navbar/30 item-center text-center backdrop-blur-md ">
                        <div class="flex-row flex item-center gap-6 p-6 w-full justify-center md:gap-10 ">
                            <img src="/images/head.png" alt="User Skin" class="w-12 h-12 mb-3 rounded shadow">
                            <div class="px-3">
                                <p class="text-1xl text-yellow-600 text-left">[Holy]</p>
                                <p class="text-white text-2xl font-bold">{{ Auth::user()->username ?? 'Guest' }}</p>
                            </div>
                            <a href="" title="Logout" class="text-2xl text-red-500 mt-5"><i
                ...
                <div class="w-full md:w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    @foreach ($ranks as $rank)
                        <div
                            ...
                            <img src="{{ asset($rank['image']) }}" alt="{{ $rank['name'] }}"
                                class="w-full h-32 object-contain mb-4 item-center">
                            <h3 class="text-lg font-semibold text-white mb-2">{{ $rank['name'] }}</h3>
                            <p class="text-gray-400 line-through font-bold">Rp
                                {{ number_format($rank['price'], 0, ',', '.') }}
                            </p>
                            <div class="flex items-center justify-center space-x-2 mb-4">
                                <p class="text-yellow-400 font-bold">Rp {{ number_format($rank['diskon'], 0, ',', '.') }}
                                </p>
                                <p class="text-red-400">-10%</p>
                            </div>
                            <button onclick="openModal({{ $rank['id'] }})"
